						...
						classes: function (column, row) {
							var anchors = $t.find('.paths').find('a'), anchor = anchors.filter(function () {
								var t = $(this),
									...
									r = t.attr('rel');
								return col == Math.abs(parseInt(column)) && r == Math.abs(parseInt(row));
							});
						...
						check: function (top, left) {
							...
							var items = $t.find('.item'),
								...
								current = items.filter(function () {
									return parseInt($(this).css('left')) == parseInt(-left);
								}),
							...
							left = parseInt(left);
							max_left = parseInt(max_left);
							top = parseInt(top);
							max_top = parseInt(max_top);
